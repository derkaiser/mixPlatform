...
* <code>ChannelEstablishMessage</code>. Common message parts are for example 
* "session key" and "payload". The entire message format can be 
* specified within the enum fields. Any 
* <code>ChannelEstablishMessagePart</code> can be retrieved or manipulated 
* using the methods 
* <code>getPart(ChannelEstablishMessagePart messagePart)</code> and 
* <code>setPart(ChannelEstablishMessagePart messagePart, byte[] data)</code> in 
* class <code>ChannelEstablishMessage</code>.
* <p>
* Overlapping message parts are supported (e. g. a field "asymmetric part", 
* which consists of several message parts itself). 
* If a <code>ChannelEstablishMessagePart</code> is of dynamic length (= its 
...
public enum ChannelEstablishMessagePart {
	...
	// header fields:
	/** 
	 * Leading zero due to (plain) RSA (first bit must be zero). (The other 7 
	 * bits of the first byte are not used in this implementation due to 
	 * readability issues).
	 */
	LEADING_ZERO 		(   0,    0,  true),
	...
	static {
		...
		for (ChannelEstablishMessagePart messagePart: values()) {
		...
		if ((headerLength % blockSize) != 0) {
			...
			LOGGER.severe(	"(MIX-MP) The accumulated length of all " 
							...
							+"class \"ChannelEstablishMessagePart\" for " 
							+"further information."
							);
	 ...
	 * dropped by the corresponding mix. For example a "session key" is "part 
	 * of the header", since it must be different for each mix and secret to 
	 * all others (and therefore won't be sent forward). In contrast, the 
	 * "payload" is not "part of the header", since it is sent from one mix to 
	 * another (after recoding). Overlapping message parts can't be headers, 
	 * of course.
	 */
	...
	/**
	 * Creates a new <code>ChannelEstablishMessagePart</code>, 
	 ...
	 * 								For example a "session key" is "part of the 
	 * 								header", since it must be different for 
	 * 								each mix and secret to all others (and 
	 * 								therefore won't be sent forward). In 
	 * 								contrast, the "payload" is not "part of the 
	 * 								header", since it is sent from one mix to 
	 * 								another (after recoding). Overlapping 
	 * 								message parts can't be headers, of course.
	 */
	private ChannelEstablishMessagePart(	int basicStartPosition, 
	...
	public int getStartPosition(int numberOfFurtherHops) {
		...
		if (!this.hasDynamicStartPosition) { // "static" parts are always at the
			...
			result +=	numberOfFurtherHops 
						* 
						ChannelEstablishMessagePart.getHeaderLength();
